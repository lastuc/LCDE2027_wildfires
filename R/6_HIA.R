...
hia <- function(expofile, pRR10 = point_RR10, sRR10 = sim_RR10, mort = mortality){
  ...
  mort_nuts <- mort_nuts[, c(list(population = sum(population)),
                         ...
                         by = .(NUTS_mort, NUTS_0, region, year, date), 
                         .SDcols = PAFcols]
  ...
  mort_nuts_year <- filter(mort_nuts_year, N >= 361) %>%
    dplyr::select(-N) %>%
...
hia24 <- hia(paste0(pathroot, "data/processed/assembled/data_2024.csv"))
hia25 <- hia(paste0(pathroot, "data/processed/assembled/data_2025.csv"))
