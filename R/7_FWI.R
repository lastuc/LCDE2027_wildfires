...
danger <- rast("data/raw/FWI/FWI_era5_glob_0_5_1979_2025.nc4") 
...
dfnuts$region <- dfnuts$EEA_subregion

# population data
population <- rast(paste0(pathroot, "data/processed/population_yearly.tif"))
names(population) <- as.character(2003:2025)
...
for(y in 1980:2025){
  ...
  yFWI <- rast(paste0(pathroot, "data/raw/FWI/FWI_era5_glob_0_5_1979_2025.nc4"),
                     lyrs = dangertimes == y)
  yFWI <- rotate(yFWI)
  crs(yFWI)  <- "epsg:4326"
